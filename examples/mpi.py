...
from pytato.array import (Array, IndexLambda,
                          make_placeholder, Placeholder, Slice, Namespace)

def show_graph(y: Array) -> None:
    """Show a graph with graphviz."""
    dot_code = pt.visualization.get_dot_graph(y)
...
from pytato.distributed import DistributedRecv, DistributedSend, make_distributed_send, make_distributed_recv
...
def main():
    ...
    bnd2 = make_distributed_send(y[0], dest_rank=(rank-1) % size, comm_tag="halo")

    halo2 = make_distributed_recv(y[9], src_rank=(rank+1) % size, comm_tag="halo",
            shape=(), dtype=float)
    y += bnd2 + halo2
    ...
    pf = PartitionFinder(nsnew, pfunc)
    new = pf(y)
