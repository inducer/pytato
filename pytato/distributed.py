...
class PartitionFinder(CopyMapper):
    ...
    def __init__(self, ns, get_partition_id:
    ...
    def map_distributed_send(self, expr, *args):
        if self.does_edge_cross_partition_boundary(expr, expr.data):
            ...
            new_binding = make_placeholder(self.ns, expr.data.shape, expr.data.dtype,
    ...
    def map_slice(self, expr, *args):
        if self.does_edge_cross_partition_boundary(expr, expr.array):
            name = self.make_new_name()
            new_binding = make_placeholder(self.ns, expr.array.shape, expr.array.dtype,
    ...
    def map_placeholder(self, expr, *args):
        ...
        for dim in expr.shape:
            if isinstance(dim, Array):
                ...
                if self.does_edge_cross_partition_boundary(expr, dim):
                    new_bindings[name] = make_placeholder(self.ns, dim.shape, dim.dtype, name,
                                                          tags=dim.tags)
        ...
        return Placeholder(namespace=self.ns, name=expr.name,
                shape=new_bindings,
                dtype=expr.dtype,
                tags=expr.tags)
    ...
    def map_index_lambda(self, expr: IndexLambda, *args) -> None:
        ...
        for child in expr.bindings.values():
            ...
            if self.does_edge_cross_partition_boundary(expr, child):
                ...
                new_bindings[name] = make_placeholder(self.ns, child.shape, child.dtype, name,
                                                      tags=child.tags)
        ...
        return IndexLambda(namespace=self.ns,expr=expr.expr,
                shape=new_shapes,
                dtype=expr.dtype,
